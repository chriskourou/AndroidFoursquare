...
import com.kourou.fouraccessq.FoursquareDialog.FsqDialogListener;
...
public class FoursquareApp {
	 ...
	 */
	public static final String CALLBACK_URL = "http://aetos.it.teithe.gr/";
		...
		try {
			...
			if (length > 0) {
				for (int i = 0; i < length; i++) {
					...
					JSONObject jvenue = (JSONObject) venues.get(i);

					FsqVenue venue = new FsqVenue();

					venue.id = jvenue.getString("id");
					venue.name = jvenue.getString("name");

					JSONObject location = (JSONObject) jvenue
					...
					try{
						venue.checkins = jvenue.getJSONObject("stats").getInt("checkinsCount");
					} catch(Exception ex){
						venue.checkins = 0;
					}
					venueList.add(venue);
